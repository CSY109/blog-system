...
publicRouter.get('/post/:slug', (req, res) => {
    ...
    try {
        ...
        const comments = db.prepare(`SELECT id, author_name, content, image_url, created_at
       FROM comments
       WHERE post_id = ? AND status = 'approved'
       ORDER BY created_at DESC`).all(post.id);
        res.json(comments);
    }
    catch (error) {
...
publicRouter.post('/', (req, res) => {
    ...
    const db = (0, index_1.getDb)();
    try {
        const result = db.prepare('INSERT INTO comments (post_id, author_name, content, image_url, status) VALUES (?, ?, ?, ?, ?)').run(post_id, author_name, content, image_url || null, 'pending');
