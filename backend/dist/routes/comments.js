...
publicRouter.get('/post/:slug', (req, res) => {
    ...
    try {
        ...
        const comments = db.prepare(`SELECT id, author_name, content, created_at
       FROM comments
...
publicRouter.post('/', (req, res) => {
    ...
    const db = (0, index_1.getDb)();
    try {
        const result = db.prepare('INSERT INTO comments (post_id, author_name, content, status) VALUES (?, ?, ?, ?)').run(post_id, author_name, content, 'pending');
